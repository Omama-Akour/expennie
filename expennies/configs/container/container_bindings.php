...
return [
    ...
    Smarty::class => function (Config $config, ContainerInterface $container) {
        $smarty = new \Smarty();
        $smarty->setTemplateDir($config->get('smarty.template_dir'));
        $smarty->setCompileDir($config->get('smarty.compile_dir'));
        $smarty->setCacheDir($config->get('smarty.cache_dir'));
